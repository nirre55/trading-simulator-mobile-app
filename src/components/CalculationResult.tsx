...
const ResultCards = memo(
  ({
    ...
    <View style={styles.cardRow}>
      <ResultCard type="min" iterations={floor} price={priceFloor} />
      <ResultCard type="max" iterations={ceil} price={priceCeil} />
    </View>
  )
);
...
const CalculationResultCard: React.FC<Props> = ({ result }) => {
  ...
  const showResultCards = Boolean(
    result.floor !== undefined &&
      result.priceFloor !== undefined &&
      result.ceil !== undefined &&
      result.priceCeil !== undefined
  );
  const showAllocation = Boolean(
